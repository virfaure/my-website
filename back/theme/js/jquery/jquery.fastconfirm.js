...
;(function ($) {
	...
	var methods = {
		...
		init: function (options) {
			...
			// VIR 
			var fastConfirmTraduction = "";
			if(fastConfirmTraduction != "") options["messages"] = fastConfirmTraduction;
			...
			$.fastConfirm = {
				defaults: {
					...
					eventToBind: false,
					action: "confirm", 			// VIR
					cancelButton: true,			// VIR
					messages: {},				// VIR 
					questionText: "Are you sure?",
					questionTextReplace: null,
					proceedText: "Yes",
					cancelText: "No",
					targetElement: null,
					unique: false,
			...
			if(params.messages[params.action] != undefined){
				
				// Traductions VIR 
				if(params.messages[params.action].questionText){
					params.questionText = params.messages[params.action].questionText;

					if(params.questionTextReplace){
						params.questionText = params.questionText.replace("questionTextReplace", params.questionTextReplace);
					}
				}

				if(params.messages[params.action].proceedText) params.proceedText = params.messages[params.action].proceedText;
				if(params.messages[params.action].cancelText) params.cancelText = params.messages[params.action].cancelText;
			}
			
			if(params.fastCustomClass) params.fastCustomClass = params.fastCustomClass+" ";
			...
			return this.each(function () {
				...
				var trigger = this,
					...
					$confirmBox = $('<div class="' + params.fastCustomClass + params.fastConfirmClass + '"><div class="' + params.fastConfirmClass + '_arrow_border"></div><div class="' + params.fastConfirmClass + '_arrow"></div><div class="' + params.fastConfirmClass + '_question">' + params.questionText + '</div><br/></div>'),
					...
					displayBox = function () {
						if (!$trigger.data('fast_confirm.box')) {
							...
							if(params.cancelButton){
								$confirmBox.append($confirmNo).append($confirmYes);
							}
