...
(function($, window, document) {
	...
	/*
	 * Variable: afnCustomFiltering
	 * Purpose:  Container for plugin filtering functions
	 * Scope:    jQuery.fn.dataTableExt
	 */
	_oExt.afnLainoaFiltering = [ ];
	...
	_oExt.oStdClasses = {
		...
		"sPageButtonStaticDisabled": "paginate_button_disabled", 	// VIR
		"sPageFirst": "first",
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function classSettings ()
		{
			 ...
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.sCookiePrefix = "LainoaLabs_";
		 ...
		 * Purpose:  Filter the input based on data and column
		 * Returns:  -
		 * Inputs:   string:sInput - string to filter the table on
		 *           int:iColumn - optional - column to limit filtering to
		 *           function:customFunction - optional - function to call for filter
		 */
		this.fnFilterLainoa = function( sInput, iColumn, customFunction )
		{
			...
			else
			{
				/* Single column filter */
				oSettings.aoPreSearchCols[ iColumn ].sSearch = sInput;
				oSettings.aoPreSearchCols[ iColumn ].customFunction = customFunction;
				_fnFilterCompleteLainoa( oSettings, oSettings.oPreviousSearch, 1 );
			}
		};
		...
		/*this.fnReloadAjax = function( oSettings, sNewSource, fnCallback, bStandingRedraw )
		{
			var oSettings = _fnSettingsFromNode( this[_oExt.iApiIndex] );
		...
		function _fnInitComplete ( oSettings, json )
		{
			...
			if(oSettings._iRecordsTotal == 0){
				// empty data => change text in tr processing withan empty tranduction sEmptyTable
				$('tbody>tr>td.'+oSettings.oClasses.sRowEmpty, oSettings.nTable).html(oSettings.oLanguage.sEmptyTable);
			}
			...
			else
			{
				     ...
				     oSettings.fnRecordsTotal() === 0 )
				{
					if(oSettings.iDraw > 1) {
						...
						nTd.innerHTML = oSettings.oLanguage.sProcessing;
					}
				}
				else
				{
		...
		/*
		 * Function: _fnFilterCompleteLainoa
		 * Purpose:  Filter the table using the column based filtering
		 * Returns:  -
		 * Inputs:   object:oSettings - dataTables settings object
		 *           object:oSearch: search information
		 */
		function _fnFilterCompleteLainoa ( oSettings, oInput )
		{
			/* Filter on everything */
			//_fnFilter( oSettings, oInput.sSearch, iForce, oInput.bRegex, oInput.bSmart );
						
			/* Custom filtering */
			if ( _oExt.afnLainoaFiltering.length !== 0 )
			{
				//_fnFilterCustom( oSettings );
			}
		...
		/*
		 * Function: _fnFilterLainoaCustom
		 * Purpose:  Apply custom filtering functions
		 * Returns:  -
		 * Inputs:   object:oSettings - dataTables settings object
		 */
		function _fnFilterLainoaCustom( oSettings )
		{
			var afnFilters = _oExt.afnLainoaFiltering;
			for ( var i=0, iLen=afnFilters.length ; i<iLen ; i++ )
			{
				var iCorrector = 0;
				for ( var j=0, jLen=oSettings.aiDisplay.length ; j<jLen ; j++ )
		...
		//this.oApi._fnReloadAjax = _fnReloadAjax;
...
/*********************************************************************************************************/
// Function called when table initComplete
var callInitComplete = function( oSettings) {
	// Change sEmptyTable with sZeroRecords
	oSettings.oLanguage.sEmptyTable = oSettings.oLanguage.sZeroRecords;
}
...
function validDate(date, hour){		
	...
	if(hour == 1) var re = /^\d{1,2}\/\d{1,2}\/\d{4}\s\d{1,2}\:\d{1,2}$/;
	else var re = /^\d{1,2}\/\d{1,2}\/\d{4}$/;
	
	if( re.test(date)){
		var adata = date.split(' ');
		
		//date
		var adataDate = adata[0];
		adataDate = adataDate.split('/');
		var day = parseInt(adataDate[0], 10);
		var month = parseInt(adataDate[1], 10);
		var year = parseInt(adataDate[2], 10);
		
		//hour
		if(hour == 1){
			var adataHour = adata[1];
			adataHour = adataHour.split(':');
			var hour = parseInt(adataHour[0], 10);
			var minute = parseInt(adataHour[1], 10)
		}else{
			var hour = 0;
			var minute = 0;
		}

		var xdata = new Date(year, month-1, day, hour, minute);
		
		if ( ( xdata.getFullYear() == year ) && ( xdata.getMonth () == month - 1 ) && ( xdata.getDate() == day ) && ( xdata.getHours() == hour ) && ( xdata.getMinutes () == minute ) )	check = xdata;
		else check = false;
	} else {check = false;}
...
/*********************************************************************************************************/
/* Sorting ASC Date like dd/mm/yyyy hh:ii */
$.fn.dataTableExt.oSort['dateLainoa-asc']  = function(a,b) {
	var x = validDate(a, 1);
	if (!x) {
...
/* Sorting DESC Date like dd/mm/yyyy hh:ii */
$.fn.dataTableExt.oSort['dateLainoa-desc']  = function(a,b) {
	var x = validDate(a, 1);
	if (!x) {
		x = 10000000000000; // = l'an 1000 ...
	}
...
$.fn.dataTableExt.oSort['dateLainoaSimple-asc']  = function(a,b) {
	var x = validDate(a, 0);
	if (!x) {
		x = 10000000000000; // = l'an 1000 ...
	}
...
/* Sorting DESC Date like dd/mm/yyyy hh:ii */
$.fn.dataTableExt.oSort['dateLainoaSimple-desc']  = function(a,b) {
	var x = validDate(a, 0);
	if (!x) {
		x = 10000000000000; // = l'an 1000 ...
	}
...
/*********************************************************************************************************/
var rangeDate = function( oSettings, aData, iDataIndex ) {
	
		// string date to check (from table and datepicker)
		var stringDateMin = $('#'+inputdateMinID).val();		//datePicker DateMin
		var stringDateMax = $('#'+inputdateMaxID).val();	 	//datePicker DateMax
		var stringDatetoCheck = aData[indexColumn]; 			//date from table
		
		//transform to object date
		var dateMin = validDate(stringDateMin);
		var dateMax = validDate(stringDateMax);
		var datetoCheck = validDate(stringDatetoCheck, 1);

		if ( dateMin == "" && dateMax == "" )
		{
			return true;
		}
		else if ( dateMin == "" && datetoCheck < dateMax )
		{
			return true;
		}
		else if ( dateMin < datetoCheck && "" == dateMax )
		...
		else if ( dateMin < datetoCheck && datetoCheck < dateMax )
		{
			return true;
		}
		return false;
...
/*
$.fn.dataTableExt.afnLainoaFiltering.push(
	rangeDate
);*/